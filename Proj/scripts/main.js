...
function LogicSim()
{
	...
	this.initialize = function()
	{
		...
		var grp = this.toolbar.addGroup("Main");
		...
		myHelpBtn = grp.addItem(new Button.Tool(images.select, function() {
				window.open('https://github.com/RobRoseKnows/logisim-browser');
		}));
	...
	this.mouseMove = function(x, y, e)
	{
		...
		dy = Math.abs(y - oldy);
		if(myIsWiring){
			if(dx > dy && myMouseDir == 1){
				myMouseDir = 0;
				lex = wdrawx;
				ley = oldy;
				dirC = 1;
			}else if (dy > dx && myMouseDir == 0){
					myMouseDir = 1;
					lex = oldx;
					ley = wdrawy;
					dirC = 1;
		...
		if(myIsWiring && dirC == 1){
			this.stopWiring(x, y);
			this.startWiring(lex, ley)
			wdrawx = lex;
				wdrawy = ley;
			dirC = 0;
		}
	...
	this.mainLoop = function(self)
	{
		...
		self.context.fillRect(128, 0, self.canvas.width - 128, self.canvas.height);
